...
const CropperRoot: React.FC<CropperRootProps> = ({
  ...
  const restrictOffset = useCallback(
    (dragOffsetX: number, dragOffsetY: number, currentZoom: number): { x: number; y: number } => {
      if (
        imageWrapperWidth <= 0 ||
  ...
  const calculateCropData = useCallback(
    (finalOffsetX?: number, finalOffsetY?: number, finalZoom?: number): Area | null => {
      const currentOffsetX =
        finalOffsetX !== undefined ? finalOffsetX : latestRestrictedOffsetRef.current.x;
      const currentOffsetY =
        finalOffsetY !== undefined ? finalOffsetY : latestRestrictedOffsetRef.current.y;
  ...
  useEffect(() => {
    if (
      ...
      if (!isInitialSetupDoneRef.current) {
        ...
        if (onCropChange)
          onCropChange(
            calculateCropData(restrictedInitial.x, restrictedInitial.y, currentZoomForSetup)
          );
        ...
        if (onCropChange)
          onCropChange(
            calculateCropData(restrictedCurrent.x, restrictedCurrent.y, currentZoomForSetup)
          );
      }
    } else {
      isInitialSetupDoneRef.current = false;
  ...
  const getCropAreaProps = useCallback(
    (): React.HTMLAttributes<HTMLDivElement> => ({
      ...
      "aria-hidden": true,
      className:
        "pointer-events-none absolute border-2 border-white shadow-[0_0_0_9999px_rgba(0,0,0,0.3)]",
    }),
    [getCropAreaStyle]
  );
...
const CropperCropArea: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  return (
    <div
      ...
      className={cn(areaProps.className, className)}
      {...restProps}
    >
      {/* For circular crop area */}
      <div className="absolute inset-0 rounded-full border-2 border-white/80 shadow-[0_0_0_9999px_rgba(0,0,0,0.5)]" />
...
const CropperDescription: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  return (
    <div id={descriptionId} className={cn("sr-only", className)} {...restProps}>
      {children ??
...
export { CropperRoot, CropperImage, CropperCropArea, CropperDescription };
