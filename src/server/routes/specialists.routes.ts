...
import { specialistRepository, type UpdateSpecialistInputType } from "@/server/repositories/specialist.repository";
...
specialistsRoutes.use("*", authMiddleware);
specialistsRoutes.use("*", requireAuth);

// Helper function for role-based field filtering
function getSpecialistFields(
  user: { id: string; role?: string | null },
  specialist: { id: string; userId: string; acuityCalendarId: string; name: string; specialty: string; location: string | null; isActive: boolean; createdAt: Date; updatedAt: Date },
...
specialistsRoutes.get("/:id", async (c) => {
  try {
    const authContext = c.get("auth");
    const user = authContext?.user;
    if (!user) {
...
specialistsRoutes.post(
  "/sync",
  requireRole("admin"),
  validateMiddleware(syncSpecialistSchema, "body"),
  async (c) => {
    try {
      const data = c.get("validatedData") as SyncSpecialistRequest;
...
specialistsRoutes.put(
  "/:id",
  requireRole("admin"),
  validateMiddleware(updateSpecialistSchema, "body"),
  async (c) => {
    try {
      const id = c.req.param("id");
      const updateData = c.get("validatedData") as UpdateSpecialistInputType;
      ...
      // Update specialist
      const updated = await specialistRepository.update(id, updateData as UpdateSpecialistInputType);
...
specialistsRoutes.get("/:id/availability", async (c) => {
  try {
    const authContext = c.get("auth");
    const user = authContext?.user;
    if (!user) {
